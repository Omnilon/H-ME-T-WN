...
export const heartbeatWorld = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (worldStatus.status === 'running') {
      if (!engine.running) {
        console.warn(`World ${worldStatus._id} is marked running but engine ${engine._id} is stopped. Restarting it.`);
        await startEngine(ctx, worldStatus.worldId);
        return;
      }

      const staleCutoff = now - ENGINE_ACTION_DURATION * 2;
      if (engine.currentTime && engine.currentTime < staleCutoff) {
        console.warn(`World ${worldStatus._id} is stale. Kicking engine ${engine._id}.`);
        await kickEngine(ctx, worldStatus.worldId);
      }
    }
  },
