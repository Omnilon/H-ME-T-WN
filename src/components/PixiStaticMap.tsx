...
export const PixiStaticMap = PixiComponent('StaticMap', {
  create: (props: { map: WorldMap; [k: string]: any }) => {
    ...
    const hasVisibleTiles = allLayers.some((layer) =>
      layer.some((column) => column.some((tileIndex) => tileIndex !== -1)),
    );

    if (map.backgroundImageUrl) {
      const background = PIXI.Sprite.from(map.backgroundImageUrl);
      background.x = 0;
      background.y = 0;
      background.width = map.width * map.tileDim;
      background.height = map.height * map.tileDim;
